...
const CompanyDetails = () => {
  ...
  const [deploymentForm, setDeploymentForm] = useState({ name: '', deployment_type: '', status: 'planned', notes: '', site_id: '', deployment_date: new Date().toISOString().split('T')[0] });
  const [amcForm, setAmcForm] = useState({ name: '', contract_number: '', start_date: '', end_date: '', coverage_type: 'comprehensive', value: '' });
      ...
      <div className="bg-white rounded-xl border border-slate-100 overflow-hidden">
        ...
        <div className="p-6">
          ...
          {activeTab === 'deployments' && (
            <div>
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-medium text-slate-900">Deployments</h3>
                <Button onClick={() => setShowAddDeployment(true)} size="sm">
                  <Plus className="h-4 w-4 mr-1" /> Add Deployment
                </Button>
              </div>
