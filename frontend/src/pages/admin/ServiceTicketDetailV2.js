...
const ScheduleVisitModal = ({ open, onClose, onConfirm, ticket }) => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`${API}/api/ticketing/engineers/${engineerId}/schedule`, { headers: authHeaders() })
      .then(r => r.json())
  ...
  const existingSlots = [...(schedule?.schedules || []), ...(schedule?.tickets || [])];
  const dateSlots = date ? existingSlots.filter(s => (s.scheduled_at || '').startsWith(date)) : [];
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center" data-testid="schedule-visit-modal">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4">
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <Input type="date" value={date} onChange={e => setDate(e.target.value)} min={new Date().toISOString().split('T')[0]} data-testid="visit-date" />
            </div>
            <div>
              <label className="text-sm font-medium block mb-1">Start Time *</label>
              <Input type="time" value={time} onChange={e => setTime(e.target.value)} data-testid="visit-time" />
            </div>
            <div>
          ...
          {date && dateSlots.length > 0 && (
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-3" data-testid="calendar-conflicts">
              <p className="text-sm font-medium text-amber-800 mb-2 flex items-center gap-1">
                <AlertTriangle className="w-4 h-4" /> Existing appointments on {date}:
              </p>
              {dateSlots.map((slot, i) => (
                <div key={i} className="text-xs text-amber-700 ml-5 mb-1">
                  {(slot.scheduled_at || '').substring(11, 16)} - {slot.ticket_number || slot.subject || 'Visit'} ({slot.company_name || slot.current_stage_name || 'N/A'})
                </div>
              ))}
            </div>
          )}
          {date && dateSlots.length === 0 && (
            <div className="bg-green-50 border border-green-200 rounded-lg p-3">
              <p className="text-sm text-green-700 flex items-center gap-1"><CheckCircle className="w-4 h-4" /> No conflicts on this date</p>
            </div>
          )}
        ...
        <div className="flex justify-end gap-3 p-5 border-t">
          <Button variant="outline" onClick={onClose}>Cancel</Button>
          <Button disabled={!date || !time} onClick={() => {
            const scheduled_at = `${date}T${time}:00`;
            const scheduled_end = endTime ? `${date}T${endTime}:00` : undefined;
