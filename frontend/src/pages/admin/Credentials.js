...
import { 
  Search, Key, Eye, EyeOff, Copy, Check, Building2, Laptop, Wifi, 
  Camera, Server, Router, Lock, Globe, Shield, Filter, ChevronDown,
  ExternalLink, Phone, User, Network
} from 'lucide-react';
...
const Credentials = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [credRes, compRes] = await Promise.all([
        axios.get(`${API}/admin/credentials`, {
          params: { credential_type: filterType !== 'all' ? filterType : undefined, company_id: filterCompany || undefined },
  ...
  }, [token, filterType, filterCompany]);
  ...
  const renderCredentialValue = (value, id, field, isPassword = false) => {
    ...
    return (
      <div className="flex items-center gap-2">
        ...
        {isPassword && (
          <button
            onClick={() => togglePasswordVisibility(id, field)}
        ...
        <button
          onClick={() => copyToClipboard(value, id, field)}
          className="p-1 hover:bg-slate-100 rounded"
        >
    ...
    <div className="space-y-6" data-testid="credentials-page">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500">View all device and network credentials in one place</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-100 p-4">
        <div className="flex flex-wrap gap-4">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              type="text"
              placeholder="Search by device, company, type..."
              value={searchQuery}
      ...
      {loading ? (
            ...
            return (
              ...
              >
                ...
                <div className="space-y-2 text-sm">
                  {isInternet ? (
                    <>
                      ...
                      {item.credentials?.static_ip && (
                        <div className="flex items-center justify-between">
                          <span className="text-slate-500">Static IP:</span>
                          <code className="bg-slate-100 px-2 py-0.5 rounded">{item.credentials.static_ip}</code>
                        </div>
                      )}
                    </>
                  ) : (
                    <>
                      ...
                      {item.credentials?.username && (
                        <div className="flex items-center justify-between">
                          ...
                          <code className="bg-slate-100 px-2 py-0.5 rounded">{item.credentials.username}</code>
                        </div>
                      )}
                    </>
                  )}
                ...
                <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-400">
                  <span>{item.location || item.site_name || 'No location'}</span>
        ...
        <div className="bg-white rounded-xl border border-slate-100 p-12 text-center">
          ...
          <p className="text-sm text-slate-400 mt-1">Add credentials to devices or internet services to see them here</p>
        </div>
      )}
            ...
            <div className="space-y-6 mt-4">
              ...
              <div className="flex items-start gap-4 p-4 bg-slate-50 rounded-lg">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-slate-500">{selectedItem.company_name}</p>
                  <div className="flex gap-4 mt-2 text-sm text-slate-500">
                    {selectedItem.device_type && <span>Type: {selectedItem.device_type}</span>}
                    {selectedItem.serial_number && <span>SN/Account: {selectedItem.serial_number}</span>}
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-3">
                  {selectedItem.source_type === 'internet' ? (
                    <>
                      ...
                      {selectedItem.credentials?.pppoe_username && (
                        <div className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                          <span className="text-slate-600">PPPoE Username</span>
                          {renderCredentialValue(selectedItem.credentials.pppoe_username, selectedItem.id, 'pppoe_username')}
                        </div>
                      )}
                      {selectedItem.credentials?.pppoe_password && (
                        <div className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                          <span className="text-slate-600">PPPoE Password</span>
                          {renderCredentialValue(selectedItem.credentials.pppoe_password, selectedItem.id, 'pppoe_password', true)}
                        </div>
                      )}
                      ...
                      {selectedItem.credentials?.gateway && (
                        <div className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                          <span className="text-slate-600">Gateway</span>
                          {renderCredentialValue(selectedItem.credentials.gateway, selectedItem.id, 'gateway')}
                        </div>
                      )}
                    </>
                  ) : (
              ...
              {selectedItem.source_type === 'internet' && selectedItem.support_phone && (
                <div>
                  ...
                  <div className="p-3 bg-slate-50 rounded-lg flex items-center gap-2">
                    ...
                    <span>{selectedItem.support_phone}</span>
                  </div>
                </div>
              )}
