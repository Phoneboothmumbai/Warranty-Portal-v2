...
class TestPendingBillsAPI:
    ...
    def test_list_pending_bills_with_billed_filter(self):
        ...
        assert res.status_code == 200
        data = res.json()
        # All returned bills should have status='billed'
        for bill in data["bills"]:
    ...
    def test_pending_bill_structure(self):
        ...
        if data["bills"]:
            ...
            # Verify items array structure if present
            if bill.get("items"):
                item = bill["items"][0]
    ...
    def test_complete_bill_requires_bill_number(self):
        ...
        # Try to complete without bill_number
    ...
    def test_complete_bill_already_billed_rejected(self):
        ...
        # Try to complete an already billed bill
        res = self.session.put(f"{BASE_URL}/api/admin/pending-bills/{bill_id}/complete", json={
    ...
    def test_settings_billing_emails_get(self):
        ...
        assert res.status_code == 200
        data = res.json()
        # billing_emails may be empty array or populated
        assert "billing_emails" in data or data.get("billing_emails") is None, "billing_emails field should be present"
...
class TestPendingBillsDataValidation:
    ...
    def test_bill_totals_calculation(self):
        ...
        for bill in bills[:3]:  # Check first 3 bills
            ...
            # Compare (allow small float differences)
